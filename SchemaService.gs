/**
 * SchemaService.gs — Preferencial - SGTE
...
// PROJECT_SCHEMA_DEFINITIONS preservado do ServiceCompatibility.gs original
// O Preferencial-SGTE define seus schemas via funcoes proprias (getSchemas/getSchema).
// Mantemos um bloco de baseline que a IIFE padrao consegue usar.
var PROJECT_SCHEMA_DEFINITIONS = {
  ...
  AUDIT_LOGS: {
    entity: 'AUDIT_LOGS', sheetName: 'LogsAuditoria',
    headers: ['ID','Timestamp','Level','Action','Entity','UserID','Message','EntityID','Details'],
    required: ['ID'], identifier: 'ID'
  },
  SCHOOLS: {
    entity: 'SCHOOLS', sheetName: 'Escolas',
    headers: ['ID','Name','Address','Latitude','Longitude','ContactPerson','ContactPhone','Status'],
    required: ['ID'], identifier: 'ID'
  },
  DRIVERS: {
    entity: 'DRIVERS', sheetName: 'Motoristas',
    headers: ['ID','Name','LicenseNumber','ContactPhone','Email','Status','CreatedAt','UpdatedAt'],
    required: ['ID'], identifier: 'ID'
  },
  VEHICLES: {
    entity: 'VEHICLES', sheetName: 'Veiculos',
    headers: ['ID','LicensePlate','Model','Type','Capacity','FuelType','FuelEfficiency','IsAccessible','Status','CreatedAt','UpdatedAt'],
    required: ['ID'], identifier: 'ID'
  },
  ROUTES: {
    entity: 'ROUTES', sheetName: 'Rotas',
    headers: ['ID','RouteName','VehicleID','DriverID','Capacity','CurrentStudents','Status','StartTime','EndTime','Distance','Duration','CurrentLatitude','CurrentLongitude','ETA','CreatedAt','UpdatedAt'],
    required: ['ID'], identifier: 'ID'
  },
  STUDENTS: {
    entity: 'STUDENTS', sheetName: 'Estudantes',
    headers: ['ID','Name','EnrollmentNumber','SchoolID','School','Address','Latitude','Longitude','AccessibilityNeeds','MedicalConditions','RouteID','FamilyContact','FamilyPhone','Status','CreatedAt','UpdatedAt'],
    required: ['ID'], identifier: 'ID'
  },
  STOP_POINTS: {
    entity: 'STOP_POINTS', sheetName: 'PontosDeParada',
    headers: ['ID','RouteID','Address','Latitude','Longitude','Order','ArrivalTime','DepartureTime','CreatedAt','UpdatedAt'],
    required: ['ID'], identifier: 'ID'
  },
  TRIPS: {
    entity: 'TRIPS', sheetName: 'Viagens',
    headers: ['ID','RouteID','VehicleID','DriverID','TripDate','TripType','StartTime','EndTime','StudentsCount','ActualDuration','ActualDistance','FuelConsumed','Status','CreatedAt','UpdatedAt'],
    required: ['ID'], identifier: 'ID'
  },
  ATTENDANCE: {
    entity: 'ATTENDANCE', sheetName: 'Frequencia',
    headers: ['ID','StudentID','RouteID','TripID','AttendanceDate','TripType','Status','IncidentDetails','RecordedBy','CreatedAt','UpdatedAt'],
    required: ['ID'], identifier: 'ID'
  },
  JOB_QUEUE: {
    entity: 'JOB_QUEUE', sheetName: 'FilaDeTrabalho',
    headers: ['JobID','JobType','Status','Payload','Result','ErrorMessage','RequestedAt','StartedAt','CompletedAt','UpdatedAt'],
    required: ['JobID'], identifier: 'JobID'
  },
  OPTIMIZATION_CONFIG: {
    entity: 'OPTIMIZATION_CONFIG', sheetName: 'ConfiguracaoOtimizacao',
    headers: ['ID','averageSpeedKmH','fuelCostPerLiter','vehicleFuelEfficiencyKmPerLiter','optimizationAlgorithm','UpdatedAt'],
    required: ['ID'], identifier: 'ID'
  },
  OPTIMIZATION_RESULTS: {
    entity: 'OPTIMIZATION_RESULTS', sheetName: 'ResultadosOtimizacao',
    headers: ['ResultID','JobID','RouteID','OriginalDuration','OptimizedDuration','OriginalDistance','OptimizedDistance','FuelSaved','TimeSaved','OptimizationDate','CreatedAt'],
    required: ['ResultID'], identifier: 'ResultID'
  },
  ROUTE_HISTORY: {
    entity: 'ROUTE_HISTORY', sheetName: 'HistoricoRotas',
    headers: ['ID','RouteID','UserID','Timestamp','ChangeDetails'],
    required: ['ID'], identifier: 'ID'
  },
  STUDENT_HISTORY: {
    entity: 'STUDENT_HISTORY', sheetName: 'HistoricoEstudantes',
    headers: ['ID','StudentID','UserID','Timestamp','ChangeDetails'],
    required: ['ID'], identifier: 'ID'
  },
  ROUTE_OPTIMIZATION_HISTORY: {
    entity: 'ROUTE_OPTIMIZATION_HISTORY', sheetName: 'HistoricoOtimizacoes',
    headers: ['ID','JobID','RouteID','OptimizationType','InputPayload','OutputResult','Timestamp'],
    required: ['ID'], identifier: 'ID'
  },
  OPTIMIZATION_LOGS: {
    entity: 'OPTIMIZATION_LOGS', sheetName: 'LogsOtimizacao',
    headers: ['ID','JobID','Timestamp','Status','InputPayload','OutputResult','ErrorMessage','DurationMs'],
    required: ['ID'], identifier: 'ID'
  },
  ROUTE_COSTS: {
    entity: 'ROUTE_COSTS', sheetName: 'CustosRotas',
    headers: ['ID','RouteID','Date','CostType','Amount','Description','CreatedAt'],
    required: ['ID'], identifier: 'ID'
  },
  ROUTE_INCIDENTS: {
    entity: 'ROUTE_INCIDENTS', sheetName: 'IncidentesRotas',
    headers: ['ID','RouteID','IncidentType','Description','Timestamp','CreatedAt'],
    required: ['ID'], identifier: 'ID'
  },
  ROUTE_SCHEDULES: {
    entity: 'ROUTE_SCHEDULES', sheetName: 'AgendaRotas',
    headers: ['ID','ScheduleDate','RouteID','DriverID','VehicleID','ScheduledStartTime','ScheduledEndTime','Status','CreatedAt','UpdatedAt'],
    required: ['ID'], identifier: 'ID'
  },
  DRIVER_AVAILABILITY: {
    entity: 'DRIVER_AVAILABILITY', sheetName: 'DisponibilidadeMotoristas',
    headers: ['ID','DriverID','Date','Status','StartTime','EndTime','CreatedAt','UpdatedAt'],
    required: ['ID'], identifier: 'ID'
  },
  EVENTS: {
    entity: 'EVENTS', sheetName: 'Eventos',
    headers: ['ID','EventType','EventDate','Description','AffectsRoutes','CreatedAt','UpdatedAt'],
    required: ['ID'], identifier: 'ID'
  },
  GEOFENCES: {
    entity: 'GEOFENCES', sheetName: 'Geocercas',
    headers: ['ID','Name','Type','Coordinates','Radius','Status','CreatedAt','UpdatedAt'],
    required: ['ID'], identifier: 'ID'
  },
  NON_BOARDING_NOTICES: {
    entity: 'NON_BOARDING_NOTICES', sheetName: 'AvisosNaoEmbarque',
    headers: ['ID','StudentID','NoticeDate','Reason','NotifiedBy','NotificationMethod','Status','CreatedAt','UpdatedAt'],
    required: ['ID'], identifier: 'ID'
  },
  NOTIFICATION_SETTINGS: {
    entity: 'NOTIFICATION_SETTINGS', sheetName: 'PreferenciasNotificacao',
    headers: ['UserID','emailEnabled','smsEnabled','nonBoardingEmail','UpdatedAt'],
    required: ['UserID'], identifier: 'UserID'
  },
  NOTIFICATION_TEMPLATES: {
    entity: 'NOTIFICATION_TEMPLATES', sheetName: 'TemplatesNotificacao',
    headers: ['ID','Name','Subject','Body','Type','Status','CreatedAt','UpdatedAt'],
    required: ['ID'], identifier: 'ID'
  },
  STUDENT_GROUPS: {
    entity: 'STUDENT_GROUPS', sheetName: 'GruposEstudantes',
    headers: ['ID','GroupName','StudentIDs','CreatedAt','UpdatedAt'],
    required: ['ID'], identifier: 'ID'
  },
  STUDENT_HEALTH: {
    entity: 'STUDENT_HEALTH', sheetName: 'SaudeEstudantes',
    headers: ['StudentID','MedicalConditions','Allergies','Medications','Notes','UpdatedAt'],
    required: ['StudentID'], identifier: 'StudentID'
  },
  STUDENT_PICKUP_DROPOFF: {
    entity: 'STUDENT_PICKUP_DROPOFF', sheetName: 'EmbarqueDesembarque',
    headers: ['ID','StudentID','Type','Latitude','Longitude','Address','IsPrimary','CreatedAt','UpdatedAt'],
    required: ['ID'], identifier: 'ID'
  },
  TRANSPORT_REQUESTS: {
    entity: 'TRANSPORT_REQUESTS', sheetName: 'SolicitacoesTransporte',
    headers: ['ID','StudentID','StudentName','Address','School','ContactInfo','RequestType','Status','RejectionReason','CreatedAt','UpdatedAt'],
    required: ['ID'], identifier: 'ID'
  },
  VEHICLE_MAINTENANCE: {
    entity: 'VEHICLE_MAINTENANCE', sheetName: 'ManutencaoVeiculos',
    headers: ['ID','VehicleID','MaintenanceType','ScheduledDate','CompletionDate','Description','Status','CreatedAt','UpdatedAt'],
    required: ['ID'], identifier: 'ID'
  },
  USER_ACTIVITY: {
    entity: 'USER_ACTIVITY', sheetName: 'AtividadesUsuarios',
    headers: ['ID','Timestamp','UserID','ActivityType','Details'],
    required: ['ID'], identifier: 'ID'
  },
  USER_SETTINGS: {
    entity: 'USER_SETTINGS', sheetName: 'PreferenciasUsuarios',
    headers: ['UserID','language','theme','UpdatedAt'],
    required: ['UserID'], identifier: 'UserID'
  },
  LOGS: {
    entity: 'LOGS', sheetName: 'Logs',
    headers: ['ID','Timestamp','Level','Message','Context','UserID'],
    required: ['ID'], identifier: 'ID'
  }
};

// Login usa a aba Usuarios com senha em texto plano
var SYNTHETIC_USERS_LOGIN_SCHEMAS = [
  { sheetName: 'Usuarios', headers: ['ID','Username','Password','Role','Status'],
...
var SchemaService = (function() {
  ...
  var SYNTHETIC_PASSWORD = 'admin123';
    ...
    try {
      return SYNTHETIC_SEED.map(function(name, i) {
        ...
        return { id: SYNTHETIC_PREFIX + pad, name: name,
                 ...
                 status: 'ativo', password: SYNTHETIC_PASSWORD };
      });
    } catch (error) {
      ...
      try {
        try {
          ...
          return { ok: true, action: 'seed-synthetic-users',
                   password: SYNTHETIC_PASSWORD, sheets: summary };
        } catch (error) {

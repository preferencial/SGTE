...
var AuthStandardService = (function () {
  ...
  function logout(token) {
    try {
      var normalizedToken = String(token == null ? '' : token).trim();
      if (normalizedToken) scriptProperties_().deleteProperty(config_.tokenPrefix + normalizedToken);
      return { ok: true };
    } catch (error) {
  ...
  function getSessionByToken_(token) {
    try {
      ...
      var key = config_.tokenPrefix + normalizedToken;
      var raw = scriptProperties_().getProperty(key);
      if (!raw) return null;
      try {
        var session = JSON.parse(raw);
        var expiresAt = Number(session && session.expiresAt);
        if (!session || !session.userId || !isFinite(expiresAt) || expiresAt <= now_()) {
